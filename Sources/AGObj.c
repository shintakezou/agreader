...
void InsertAGWord(AGPara par, AGWord new, AGWord ins)
{
	PREV(new) = ins;
	NEXT(new) = NULL;
	if(ins != NULL) NEXT(ins) = new;
	else if(par)    par->line = new;
}
...
AGWord NewWord(AGPara par, AGWord old, char *data, AGContext *agc)
{
	...
	if( NULL != (new = (AGWord) malloc(sizeof(*new))))
	{
		...
		new->data  = data;
		new->link  = NULL;
		if(agc) memcpy(&new->fgpen, &agc->fgpen, 3);
	} else quit(MemErr, QUIT_ERROR);
	return new;
}
...
AGWord FreeSplit( AGWord word )
{
	...
	if( ((AGSplit)word)->dat ) ((AGSplit)word)->dat[0] = ((AGSplit)word)->svg;
	free(word);
	return next;
}
